...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex text-[#1d2939] font-sans relative overflow-hidden">
      <div className="app-subtle-grid fixed inset-0 pointer-events-none opacity-80" />

      {isSidebarOpen && (
        <button
          type="button"
          className="fixed inset-0 z-20 bg-[#1d2939]/30 backdrop-blur-sm lg:hidden"
          aria-label="필터 메뉴 닫기"
      ...
      <main className="app-main flex-1 min-h-screen px-4 sm:px-6 lg:px-10 xl:px-12 py-5 lg:py-8 flex flex-col relative z-10">
        <button
          type="button"
          className="mobile-filter-trigger mb-4 inline-flex w-fit items-center gap-2 rounded-lg border border-[#d0d5dd] bg-white px-3 py-2 text-xs font-bold text-[#1d2939] shadow-sm"
          onClick={() => setIsSidebarOpen(true)}
        >
          <Menu className="h-4 w-4 text-[#2b5c7e]" />
          필터 열기
          {activeFilterCount > 0 && (
            <span className="rounded-md bg-[#2b5c7e] px-1.5 py-0.5 text-[0.65rem] text-white">
              {activeFilterCount}
            </span>
          )}
        </button>
        ...
        <section className={`w-full glass-card bg-gradient-to-r ${heroHighlight.themeColor} border p-5 sm:p-6 lg:p-7 flex flex-col md:flex-row items-start md:items-center gap-6 mb-7 overflow-hidden relative group select-none animate-fade-in`}>
          <div className="w-28 sm:w-32 md:w-40 shrink-0 rounded-lg overflow-hidden shadow-[0_8px_20px_rgba(29,41,57,0.12)] border border-[#d8dee6] transition-transform duration-300 group-hover:scale-[1.01]">
            <img
              ...
              onError={(e) => {
                ...
                e.target.parentNode.innerHTML = `
                  <div class="w-full aspect-[3/4] bg-[#f7f8fa] flex flex-col items-center justify-center p-4 text-center border-l-4 border-[#2b5c7e]">
                    <div class="p-2.5 bg-[#e8f1f6] rounded-full mb-3 text-[#2b5c7e]">
                      <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                    </div>
                    <span class="text-[0.62rem] font-bold text-[#2b5c7e] tracking-widest uppercase block mb-1">BRAIN & MIND</span>
                    <span class="text-xs font-bold text-[#1d2939]">${heroHighlight.subtitle}</span>
                  </div>
                `;
              }}
            />
          ...
          <div className="flex-1 flex flex-col gap-3.5">
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <span className="text-[0.7rem] font-bold text-[#597c74] flex items-center gap-1">
                <Sparkles className="w-3.5 h-3.5 text-[#597c74]" />
                {heroHighlight.accent}
              </span>
            </div>
            
            <h3 className="text-xl md:text-3xl font-extrabold tracking-tight text-[#1d2939] leading-tight">
              {heroHighlight.title}
            </h3>
            
            <p className="text-[0.825rem] text-[#475467] leading-relaxed max-w-2xl font-normal">
              {heroHighlight.description}
            </p>

            <div className="grid grid-cols-2 sm:flex sm:items-center gap-3 sm:gap-5 mt-1 border-t border-[#d9e0e7] pt-4 text-[0.725rem] text-[#667085] font-semibold">
              <span className="flex items-center gap-2 rounded-lg bg-[#f3f6f8] px-3 py-2">
                <Newspaper className="w-4 h-4 text-[#2b5c7e]" />
                {issueArticles.length}개 원고
              </span>
              <span className="flex items-center gap-2 rounded-lg bg-[#f3f6f8] px-3 py-2">
                <Brain className="w-4 h-4 text-[#597c74]" />
                {availableTags.length}개 태그
              </span>
              <span className="flex items-center gap-2 rounded-lg bg-[#f3f6f8] px-3 py-2">
                <GraduationCap className="w-4 h-4 text-[#a66a3f]" />
                총 {totalPages}p
              </span>
        ...
        {activeFilterCount > 0 && (
          <div className="mb-5 flex flex-wrap items-center gap-2 text-[0.72rem] text-[#475467]">
            <span className="font-bold text-[#667085]">적용된 필터</span>
            {searchQuery.trim() && (
              <span className="rounded-md border border-[#c9dbe8] bg-[#e8f1f6] px-2.5 py-1 text-[#2b5c7e]">
                검색: {searchQuery.trim()}
              </span>
            )}
            {selectedCategories.map((category) => (
              <span key={category} className="rounded-md border border-[#d9e0e7] bg-white px-2.5 py-1">
                {category}
              </span>
            ))}
            {selectedTags.map((tag) => (
              <span key={tag} className="rounded-md border border-[#cfe2dc] bg-[#edf5f2] px-2.5 py-1 text-[#426b63]">
                #{tag}
              </span>
            ))}
            <button
              ...
              className="inline-flex items-center gap-1 rounded-md border border-[#d0d5dd] bg-white px-2.5 py-1 font-bold text-[#475467] hover:border-[#2b5c7e] hover:text-[#1d2939]"
            >
        ...
        {filteredArticles.length > 0 ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center py-24 px-5 text-center glass-card border-[#d9e0e7] mb-16 select-none animate-fade-in">
            <div className="p-5 bg-[#e8f1f6] border border-[#c9dbe8] rounded-full text-[#2b5c7e] mb-4">
              <Brain className="w-12 h-12" />
            </div>
            <h4 className="text-sm font-bold text-[#1d2939] mb-1.5">
              일치하는 논문을 찾을 수 없습니다
            </h4>
